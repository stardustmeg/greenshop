...
export class Store<S, A> implements ReduxStore<S, A> {
  ...
  constructor(initialData: S, rootReducer: Reducer<S, A>) {
    ...
    // If you have unexpected bugs related to State of the app, or you need to clear out Local Storage to start afresh,
    // comment out the next line, go to the browser tab, clear out the storage manually,
    // and update the page one more time. Then come back here and uncomment it
